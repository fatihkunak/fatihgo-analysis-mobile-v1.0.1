...
import { loadYoutubeApiKey, fetchVideoData, extractVideoId, convertLiveToVideoUrl } from "@/lib/youtube-api";
...
export default function HomeScreen() {
  ...
  // Load YouTube API key when screen is focused
  useFocusEffect(
    useCallback(() => {
  ...
  const handleAnalyze = async () => {
    ...
    setIsLoading(true);
    try {
      // Convert live URLs to standard format and extract video ID
      const processedUrl = convertLiveToVideoUrl(url.trim());
      console.log("İşlenmiş URL:", processedUrl);
      if (processedUrl !== url.trim()) {
        setUrl(processedUrl); // Update the UI with the converted URL
      }

      const videoId = extractVideoId(processedUrl);
      console.log("Ayıklanan Video ID:", videoId);
      if (!videoId) {
      ...
      try {
        ...
        Alert.alert(
          "API Hatası",
          `${apiError instanceof Error ? apiError.message : "Bağlantı kurulamadı."}\n\nNot: Mock veriler kullanılıyor.`,
          [{ text: "Tamam" }]
        );
      }
