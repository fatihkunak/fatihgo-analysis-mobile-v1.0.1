...
/**
 * Safe development-only logger that is stripped from production builds.
 * React Native's __DEV__ is false in release builds, so these logs
 * will be tree-shaken by the Metro bundler.
 * Uses typeof check to avoid ReferenceError if __DEV__ is not defined.
 */
const devLog =
...
export async function apiCall<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  if (Platform.OS !== "web") {
    ...
    devLog.info("[API] apiCall", { endpoint, method: options.method || "GET" });
    if (sessionToken) {
      headers["Authorization"] = `Bearer ${sessionToken}`;
    }
  } else {
    devLog.info("[API] apiCall", { endpoint, platform: "web", method: options.method || "GET" });
  }
  ...
  devLog.info("[API] Request", { endpoint });
  ...
  try {
    ...
    devLog.info("[API] Response status:", response.status);

    if (!response.ok) {
      const errorText = await response.text();
      devLog.error("[API] Error response:", errorText);
    ...
    if (contentType && contentType.includes("application/json")) {
      ...
      devLog.info("[API] JSON response received");
      return data as T;
    }

    const text = await response.text();
    devLog.info("[API] Text response received");
    return (text ? JSON.parse(text) : {}) as T;
  } catch (error) {
    devLog.error("[API] Request failed:", error);
...
export async function exchangeOAuthCode(
  ...
  devLog.info("[API] Exchanging OAuth code");
  ...
  devLog.info("[API] OAuth exchange result", {
