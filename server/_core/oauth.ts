...
/** Safe logger that redacts sensitive data in production */
const log = {
...
function sanitizeLogData(data: Record<string, unknown>): Record<string, unknown> {
  const sensitiveKeys = ["code", "state", "token", "session", "secret", "password", "key", "authorization"];
...
export function registerOAuthRoutes(app: Express) {
  app.get("/api/oauth/callback", async (req: Request, res: Response) => {
    ...
    try {
      ...
      log.info("[OAuth] Token received, fetching user info");
  ...
  app.get("/api/oauth/mobile", async (req: Request, res: Response) => {
    ...
    try {
      ...
      log.error("[OAuth] Mobile exchange failed", error);
